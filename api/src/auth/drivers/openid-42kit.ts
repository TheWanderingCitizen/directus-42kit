...
export class OpenID42KitAuthDriver extends LocalAuthDriver {
	...
	constructor(options: AuthDriverOptions, config: Record<string, any>) {
		...
		this.client = new Promise((resolve, reject) => {
			Issuer.discover(issuerUrl)
				...
				.catch((e) => {
					logger.error(e, '[OpenID 42Kit] Failed to fetch provider config');
					process.exit(1);
				});
		});
	}
	...
	override async getUserID(payload: Record<string, any>): Promise<string> {
		...
		if (!payload['code'] || !payload['codeVerifier'] || !payload['state']) {
			logger.warn('[OpenID 42Kit] No code, codeVerifier or state in payload');
		...
		if (!identifier) {
			logger.warn(`[OpenID 42Kit] Failed to find user identifier for provider "${provider}"`);
			throw new InvalidCredentialsError();
		}
		...
		const userPayload = {
			...
			nickname: userInfo['displayName'] ?? userInfo['name'],
			email: email,
		...
		if (!allowPublicRegistration || !isEmailVerified) {
			logger.warn(`[OpenID 42Kit] User doesn't exist, and public registration not allowed for provider "${provider}"`);
			throw new InvalidCredentialsError();
		}
		...
		try {
			...
			if (isDirectusError(e, ErrorCode.RecordNotUnique)) {
				logger.warn(e, '[OpenID 42Kit] Failed to register user. User not unique');
	...
	override async refresh(user: User): Promise<void> {
		...
		if (typeof authData === 'string') {
			try {
				...
				logger.warn(`[OpenID 42Kit] Session data isn't valid JSON: ${authData}`);
			}
		}
...
const handleError = (e: any) => {
	...
	if (e instanceof errors.OPError) {
		if (e.error === 'invalid_grant') {
			// Invalid token
			logger.warn(e, `[OpenID 42Kit] Invalid grant`);
			return new InvalidTokenError();
		}

		// Server response error
		logger.warn(e, `[OpenID 42Kit] Unknown OP error`);
		...
		logger.warn(e, `[OpenID 42Kit] Unknown RP error`);
		return new InvalidCredentialsError();
	}

	logger.warn(e, `[OpenID 42Kit] Unknown error`);
	return e;
};

export function createOpenID42KitAuthRouter(providerName: string): Router {
	const env = useEnv();
	const router = Router();
	...
	router.get(
		...
		asyncHandler(async (req, res) => {
			const provider = getAuthProvider(providerName) as OpenID42KitAuthDriver;
	...
	router.get(
		...
		asyncHandler(async (req, res, next) => {
			...
			try {
				...
				logger.warn(e, `[OpenID 42Kit] Couldn't verify OpenID cookie`);
				throw new InvalidCredentialsError();
			}
			...
			try {
				...
				if (redirect) {
					...
					if (isDirectusError(error)) {
						...
						logger.warn(error, `[OpenID 42Kit] Unexpected error during OpenID login`);
					}
